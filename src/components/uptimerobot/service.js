...
function monitorServiceFn($http, $q) {
	...
	return function() {
		...
		for (var index = 0; index < max; index++) {
			results.push($http.get(`https://api.uptimerobot.com/getMonitors?apiKey=${ config.monitors[index] }&responseTimesAverage=30&format=json&logs=1&customUptimeRatio=1-2-7-14-30&responseTimes=1&noJsonCallback=1`));
		}
